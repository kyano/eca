...
(deftest normalize-messages-test
  ...
  (testing "user turn with functionResponse and text orders text before functionResponse"
    (is (= [{:role "user"
             :parts [{:text "Please explain the results"}
           ...
           (#'llm-providers.gemini/normalize-messages
            [{:role "tool_call_output"
              :content {:id "call_1"
                        ...
                        :output {:contents [{:type :text :text "file1.txt"}]}}}
             {:role "user"
              :content "Please explain the results"}])))
    (is (= [{:role "user"
             :parts [{:text "Please explain the results"}
                     {:functionResponse {:name "search_files"
                                         :response {:result "file1.txt\n"}}}]}]
...
(deftest prepare-body-continuation-turn-test
  (testing "prepare-body appends a synthetic user continuation turn when conversation ends on an assistant turn"
    (let [body (#'llm-providers.gemini/prepare-body
                {:messages [{:role "user" :content "Hello"}
  ...
  (testing "prepare-body appends a synthetic user continuation turn when conversation ends on a reason turn"
    (let [body (#'llm-providers.gemini/prepare-body
                {:messages [{:role "user" :content "Solve this"}
                            {:role "reason" :content {:text "Let me think"}}]})]
      (is (= [{:role "user" :parts [{:text "Solve this"}]}
              {:role "model" :parts [{:text "Let me think" :thought true}]}
              {:role "user" :parts [{:text "Continue"}]}]
             (:contents body))))))

(deftest prepare-body-drops-dangling-tool-call-test
  (testing "prepare-body drops dangling tool_call without tool_call_output"
    (let [body (#'llm-providers.gemini/prepare-body
                {:messages [{:role "user" :content "search files"}
                            {:role "tool_call" :content {:id "call_dangling"
                                                         :full-name "search"
                                                         :arguments {:q "clojure"}}}]})]
      (is (= [{:role "user" :parts [{:text "search files"}]}]
             (:contents body)))))

  (testing "prepare-body drops dangling tool_call_output without matching tool_call"
    (let [body (#'llm-providers.gemini/prepare-body
                {:messages [{:role "user" :content "hello"}
                            {:role "tool_call_output" :content {:id "call_orphan"
                                                                :full-name "search"
                                                                :output {:contents [{:type :text :text "result"}]}}}]})]
      (is (= [{:role "user" :parts [{:text "hello"}]}]
             (:contents body)))))

  (testing "prepare-body retains paired tool_call and drops dangling tool_call"
    (let [body (#'llm-providers.gemini/prepare-body
                {:messages [{:role "user" :content "search files"}
                            {:role "tool_call" :content {:id "call_1"
                                                         :full-name "search"
                                                         :arguments {:q "clojure"}}}
                            {:role "tool_call_output" :content {:id "call_1"
                                                                :full-name "search"
                                                                :output {:contents [{:type :text :text "found"}]}}}
                            {:role "tool_call" :content {:id "call_dangling"
                                                         :full-name "read"
                                                         :arguments {:path "foo"}}}]})]
      (is (= [{:role "user" :parts [{:text "search files"}]}
              {:role "model" :parts [{:functionCall {:name "search" :args {:q "clojure"}}}]}
              {:role "user" :parts [{:functionResponse {:name "search" :response {:result "found\n"}}}]}]
             (:contents body))))))

(deftest prepare-body-drops-leading-model-turn-test
  (testing "prepare-body drops leading model turn so it starts with a user turn"
    (let [body (#'llm-providers.gemini/prepare-body
                {:messages [{:role "assistant" :content "Welcome to assistant"}
                            {:role "user" :content "Hello"}]})]
      (is (= [{:role "user" :parts [{:text "Hello"}]}]
             (:contents body)))))

  (testing "prepare-body with only model turn drops leading model and falls back to Hello user turn"
    (let [body (#'llm-providers.gemini/prepare-body
                {:messages [{:role "assistant" :content "Welcome to assistant"}]})]
      (is (= [{:role "user" :parts [{:text "Hello"}]}]
             (:contents body)))))

  (testing "prepare-body with empty user content drops empty parts and falls back to Hello user turn"
    (let [body (#'llm-providers.gemini/prepare-body
...
(deftest normalize-messages-parallel-tool-calls-test
  (testing "parallel tool calls in same tool turn are reordered, merged into a single model turn and user turn, and only the first functionCall part retains thoughtSignature"
    (let [messages [{:role "user" :content "search"}
                    {:role "tool_call" :content {:id "c1" :full-name "search" :arguments {:q "a"} :thought-signature "sig-1" :tool-turn-id "t1"}}
                    {:role "tool_call_output" :content {:id "c1" :output {:contents [{:type :text :text "r1"}]} :tool-turn-id "t1"}}
                    {:role "tool_call" :content {:id "c2" :full-name "lookup" :arguments {:k "b"} :tool-turn-id "t1"}}
                    {:role "tool_call_output" :content {:id "c2" :output {:contents [{:type :text :text "r2"}]} :tool-turn-id "t1"}}]
          result (#'llm-providers.gemini/normalize-messages messages)]
      (is (= [{:role "user" :parts [{:text "search"}]}
              {:role "model"
               :parts [{:functionCall {:name "search" :args {:q "a"}} :thoughtSignature "sig-1"}
                       {:functionCall {:name "lookup" :args {:k "b"}}}]}
              {:role "user"
               :parts [{:functionResponse {:name "search" :response {:result "r1\n"}}}
                       {:functionResponse {:name "lookup" :response {:result "r2\n"}}}]}]
             result))
      (let [model-parts (get-in result [1 :parts])]
        (is (= "sig-1" (:thoughtSignature (first model-parts))))
        (is (nil? (:thoughtSignature (second model-parts))))
        (is (not (contains? (second model-parts) :thoughtSignature)))))))

(deftest normalize-messages-sequential-tool-calls-test
  (testing "sequential tool calls across different tool turns are kept separate and not merged into a single parallel turn"
    (let [messages [{:role "user" :content "search files and read"}
                    {:role "tool_call" :content {:id "c1" :full-name "search" :arguments {:q "a"} :thought-signature "sig-1" :tool-turn-id "t1"}}
                    {:role "tool_call_output" :content {:id "c1" :output {:contents [{:type :text :text "r1"}]} :tool-turn-id "t1"}}
                    {:role "tool_call" :content {:id "c2" :full-name "lookup" :arguments {:k "b"} :thought-signature "sig-2" :tool-turn-id "t2"}}
                    {:role "tool_call_output" :content {:id "c2" :output {:contents [{:type :text :text "r2"}]} :tool-turn-id "t2"}}]
          result (#'llm-providers.gemini/normalize-messages messages)]
      (is (= [{:role "user" :parts [{:text "search files and read"}]}
              {:role "model"
               :parts [{:functionCall {:name "search" :args {:q "a"}} :thoughtSignature "sig-1"}]}
              {:role "user"
               :parts [{:functionResponse {:name "search" :response {:result "r1\n"}}}]}
              {:role "model"
               :parts [{:functionCall {:name "lookup" :args {:k "b"}} :thoughtSignature "sig-2"}]}
              {:role "user"
               :parts [{:functionResponse {:name "lookup" :response {:result "r2\n"}}}]}]
             result))
      (is (= 5 (count result)))
      (is (= ["user" "model" "user" "model" "user"] (mapv :role result))))))

(deftest chat!-streaming-thought-signature-fallback-to-first-tool-call-test
  (testing "thoughtSignature from reasoning chunk falls back only to the first tool call, subsequent parallel tool call receives nil"
    (let [events* (atom [])
          tools-called* (atom nil)
          stream (sse-stream
                  {:candidates [{:content {:parts [{:thought true
                                                    :text "Let me search and lookup"
      ...
      (with-redefs [http/post (fn [_url _opts]
        ...
        (let [callbacks (assoc (collecting-callbacks events*)
                               :on-tools-called
                               (fn [calls]
                                 (reset! tools-called* calls)
                                 nil))]
          (llm-providers.gemini/chat!
           {:model "gemini-2.5-flash"
            :user-messages [{:role "user" :content "search and lookup"}]
            :api-key "key"
            :reason? true}
           callbacks)))
      (is (= 2 (count @tools-called*)))
      (let [[tool-1 tool-2] @tools-called*]
        (is (= "search" (:full-name tool-1)))
        (is (= "sig-456" (:thought-signature tool-1)))
        (is (= "sig-456" (:external-id tool-1)))
        (is (= "lookup" (:full-name tool-2)))
        (is (nil? (:thought-signature tool-2)))
        (is (nil? (:external-id tool-2)))
        (is (some? (:tool-turn-id tool-1)))
        (is (= (:tool-turn-id tool-1) (:tool-turn-id tool-2)))))))
